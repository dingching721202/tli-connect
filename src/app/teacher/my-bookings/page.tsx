...
import { dashboardService, leaveService } from '@/services/dataService';
import { } from '@/types';
import { getCourseLinksFromBooking } from '@/utils/courseLinksUtils';

const {
  FiCalendar, FiClock, FiUsers, FiExternalLink,
  FiX, FiEye, FiCheckCircle, FiAlertCircle, FiBook, FiUserCheck, FiMessageSquare
} = FiIcons;
...
export default function TeacherMyBookingsPage() {
  const { user } = useAuth();
  ...
  const loadUserBookings = useCallback(async (showLoading = true) => {
    if (!user || !user?.roles.some(role => ['TEACHER'].includes(role))) {
  ...
  }, [user, convertTeacherBookingData]);
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600">
                          ...
                          <div className="flex items-center space-x-1">
                            <SafeIcon icon={booking.leaveReason ? FiUserCheck : FiUsers} className="text-xs" />
                            <span>
