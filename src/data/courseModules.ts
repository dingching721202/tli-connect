import type { CourseModule } from '@/types/business';

// ========================================
// 課程模組資料 - MECE架構
// 定義課程範本和基本資訊，不包含排程相關資料
// ========================================

export const courseModules: CourseModule[] = [
  {
    id: 1,
    title: "基礎英文會話",
    description: "適合初學者的英文會話課程，重點培養日常對話能力",
    cover_image_url: "/images/courses/basic-english.jpg",
    language: "english",
    level: "beginner",
    categories: ["語言學習", "英文"],
    tags: ["英文", "會話", "初學者", "小班制"],
    total_sessions: 16,
    session_duration_minutes: 120,
    materials: [
      {
        id: 1,
        type: "DOCUMENT",
        title: "英文會話教材",
        url: "/materials/basic-english-textbook.pdf",
        order: 1,
        is_required: true
      },
      {
        id: 2,
        type: "AUDIO",
        title: "聽力練習CD",
        url: "/materials/basic-english-audio.mp3",
        order: 2,
        is_required: true
      }
    ],
    prerequisites: ["無"],
    learning_objectives: [
      "掌握基本日常會話",
      "建立英語口說信心",
      "學會常用生活詞彙"
    ],
    refund_policy: "開課前7天可全額退費",
    is_active: true,
    status: "published",
    created_at: "2025-07-14T12:00:00+00:00",
    updated_at: "2025-07-20T00:00:00+00:00"
  },
  {
    id: 2,
    title: "基礎中文會話",
    description: "專為外國學生設計的中文會話課程，從基礎發音開始",
    cover_image_url: "/images/courses/chinese-conversation.jpg",
    language: "chinese",
    level: "beginner",
    categories: ["語言學習", "中文"],
    tags: ["中文", "會話", "初學者", "發音訓練"],
    total_sessions: 20,
    session_duration_minutes: 120,
    materials: [
      {
        id: 3,
        type: "DOCUMENT",
        title: "中文會話教材",
        url: "/materials/chinese-conversation-textbook.pdf",
        order: 1,
        is_required: true
      },
      {
        id: 4,
        type: "AUDIO",
        title: "聽力練習CD",
        url: "/materials/chinese-conversation-audio.mp3",
        order: 2,
        is_required: true
      }
    ],
    prerequisites: ["無"],
    learning_objectives: [
      "掌握中文基礎發音",
      "學會日常會話表達",
      "認識基本漢字"
    ],
    refund_policy: "開課前5天可退費80%",
    is_active: true,
    status: "published",
    created_at: "2025-07-14T12:00:00+00:00",
    updated_at: "2025-07-20T00:00:00+00:00"
  },
  {
    id: 3,
    title: "商務英語進階",
    description: "針對商務環境設計的英語課程，包含會議、簡報、談判技巧",
    cover_image_url: "/images/courses/business-english.jpg",
    language: "english",
    level: "intermediate",
    categories: ["商務英語", "職場溝通"],
    tags: ["商務英語", "職場溝通", "簡報技巧", "談判"],
    total_sessions: 24,
    session_duration_minutes: 120,
    materials: [
      {
        id: 5,
        type: "DOCUMENT",
        title: "商務英語教材",
        url: "/materials/business-english-textbook.pdf",
        order: 1,
        is_required: true
      },
      {
        id: 6,
        type: "LINK",
        title: "線上練習平台",
        url: "https://platform.business-english.com",
        order: 2,
        is_required: true
      }
    ],
    prerequisites: ["基礎英語能力"],
    learning_objectives: [
      "掌握商務會議英語",
      "學會專業簡報技巧",
      "提升商務談判能力"
    ],
    refund_policy: "開課前7天可全額退費",
    is_active: true,
    status: "published",
    created_at: "2025-07-14T12:00:00+00:00",
    updated_at: "2025-07-20T00:00:00+00:00"
  },
  {
    id: 4,
    title: "日語入門",
    description: "日語五十音開始，包含基礎語法與日常會話",
    cover_image_url: "/images/courses/japanese-basic.jpg",
    language: "japanese",
    level: "beginner",
    categories: ["語言學習", "日語"],
    tags: ["日語", "五十音", "入門", "基礎語法"],
    total_sessions: 28,
    session_duration_minutes: 120,
    materials: [
      {
        id: 7,
        type: "DOCUMENT",
        title: "日語入門教材",
        url: "/materials/japanese-basic-textbook.pdf",
        order: 1,
        is_required: true
      },
      {
        id: 8,
        type: "AUDIO",
        title: "發音練習CD",
        url: "/materials/japanese-pronunciation-audio.mp3",
        order: 2,
        is_required: true
      }
    ],
    prerequisites: ["無"],
    learning_objectives: [
      "熟練掌握五十音",
      "學會基礎語法結構",
      "能進行簡單日常對話"
    ],
    refund_policy: "開課前5天可退費80%",
    is_active: true,
    status: "published",
    created_at: "2025-07-14T12:00:00+00:00",
    updated_at: "2025-07-20T00:00:00+00:00"
  },
  {
    id: 5,
    title: "TOEIC 衝刺班",
    description: "針對TOEIC考試的密集訓練課程，目標分數700+",
    cover_image_url: "/images/courses/toeic-prep.jpg",
    language: "english",
    level: "intermediate",
    categories: ["證照考試", "英語檢定"],
    tags: ["TOEIC", "證照考試", "密集班", "考試技巧"],
    total_sessions: 12,
    session_duration_minutes: 180,
    materials: [
      {
        id: 9,
        type: "DOCUMENT",
        title: "TOEIC官方指南",
        url: "/materials/toeic-official-guide.pdf",
        order: 1,
        is_required: true
      },
      {
        id: 10,
        type: "QUIZ",
        title: "模擬試題",
        url: "/materials/toeic-practice-tests.pdf",
        order: 2,
        is_required: true
      }
    ],
    prerequisites: ["中級英語程度"],
    learning_objectives: [
      "掌握TOEIC考試技巧",
      "提升聽力理解能力",
      "強化閱讀速度與準確度"
    ],
    refund_policy: "開課前3天可退費50%",
    is_active: true,
    status: "published",
    created_at: "2025-07-14T12:00:00+00:00",
    updated_at: "2025-07-20T00:00:00+00:00"
  },
  {
    id: 6,
    title: "雅思寫作專修",
    description: "專攻IELTS寫作技巧，提升學術寫作能力",
    cover_image_url: "/images/courses/ielts-writing.jpg",
    language: "english",
    level: "advanced",
    categories: ["證照考試", "學術英語"],
    tags: ["IELTS", "雅思", "寫作", "學術英語"],
    total_sessions: 16,
    session_duration_minutes: 150,
    materials: [
      {
        id: 11,
        type: "DOCUMENT",
        title: "雅思寫作指南",
        url: "/materials/ielts-writing-guide.pdf",
        order: 1,
        is_required: true
      },
      {
        id: 12,
        type: "DOCUMENT", 
        title: "範文集",
        url: "/materials/ielts-sample-essays.pdf",
        order: 2,
        is_required: true
      }
    ],
    prerequisites: ["高中級英語程度"],
    learning_objectives: [
      "掌握雅思寫作技巧",
      "提升學術寫作水準",
      "學會論證結構組織"
    ],
    refund_policy: "開課前7天可全額退費",
    is_active: true,
    status: "published",
    created_at: "2025-07-14T12:00:00+00:00",
    updated_at: "2025-07-20T00:00:00+00:00"
  }
];

// ========================================
// 輔助函數
// ========================================

// 根據語言獲取課程模組
export const getCourseModulesByLanguage = (language: string): CourseModule[] => {
  return courseModules.filter(module => module.language === language);
};

// 根據等級獲取課程模組
export const getCourseModulesByLevel = (level: string): CourseModule[] => {
  return courseModules.filter(module => module.level === level);
};

// 根據分類獲取課程模組
export const getCourseModulesByCategory = (category: string): CourseModule[] => {
  return courseModules.filter(module => 
    module.categories.includes(category)
  );
};

// 根據ID獲取課程模組
export const getCourseModuleById = (id: number): CourseModule | undefined => {
  return courseModules.find(module => module.id === id);
};

// 檢查課程模組是否有效
export const isActiveCourseModule = (module: CourseModule): boolean => {
  return module.is_active;
};

// 搜尋課程模組
export const searchCourseModules = (keyword: string): CourseModule[] => {
  const searchTerm = keyword.toLowerCase();
  return courseModules.filter(module =>
    module.title.toLowerCase().includes(searchTerm) ||
    module.description.toLowerCase().includes(searchTerm) ||
    module.tags.some(tag => tag.toLowerCase().includes(searchTerm))
  );
};

// 向下相容的預設匯出
export default courseModules;